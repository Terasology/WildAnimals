 ...
 * <p>
 * An animal may have multiple prefabs in its lifecycle.  Each prefab except the last should have a {@link
 * WildAnimalGrowthComponent} giving the next prefab in the chain, along with the range of time to spend in this prefab.
 *  At a random time within the specified range (since the growth component was activated, typically immediately on
 * spawning), the existing animal is deleted and replaced with a freshly spawned instance of the new prefab.
 *
 * <hr>
 *
 * <b>Example:</b> Suppose we have a lizard, and we want to model its
 * maturation process as a three-stage cycle, with {@code lizardEgg}, {@code babyLizard}, and {@code lizard} prefabs.
 * We want each lizard to spend 10-15 minutes as an egg, then 5-10 minutes as a baby, before finally maturing.  Then the
 * prefabs should have growth components as follows:
 * <p>
 * lizardEgg.prefab: {@code "WildAnimalGrowth": { "minGrowthTime": 600000, "maxGrowthTime": 900000, "nextStagePrefab":
 * "WildAnimals:babyLizard" } }
 * <p>
 * babyLizard.prefab: {@code "WildAnimalGrowth": { "minGrowthTime": 300000, "maxGrowthTime": 600000, "nextStagePrefab":
 * "WildAnimals:lizard" } }
 * <p>
...
public class WildAnimalsGrowthSystem extends BaseComponentSystem {
    ...
    public void onGrowthComponentActivated(OnActivatedComponent event, EntityRef entityRef,
                                           WildAnimalGrowthComponent growthComponent) {
     ...
     */
    @ReceiveEvent(components = {WildAnimalComponent.class})
    public void onGrowth(DelayedActionTriggeredEvent event, EntityRef entityRef,
                         WildAnimalGrowthComponent growthComponent) {
